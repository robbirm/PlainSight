...
let foundSecrets1 = 0;
const foundQMs1 = JSON.parse(localStorage.getItem('foundQMs_level2.1')) || {};
const gnomeWasClicked1 = localStorage.getItem('gnomeUnlocked_level2.1') === 'true';
...
function showGnome1() {
  if (!document.getElementById('gnomeReveal1')) {
    ...
    gnomeImage1.src = 'backgrounds/level2/gnome1.png';
...
// clicking on gnome vvv
    ...
    gnomeImage1.addEventListener('click', () => {
      if (!localStorage.getItem('gnomeUnlocked_level2.1')) {
        localStorage.setItem('gnomeUnlocked_level2.1', 'true');

        const gnomes = JSON.parse(localStorage.getItem('gnomes')) || {};
        gnomes['level2.1'] = true;
...
// clicking on gnome ^^^	
    ...
    if (localStorage.getItem('gnomeUnlocked_level2.1') === 'true') {
      gnomeImage1.classList.add('found');
...
let foundSecrets2 = 0;
const foundQMs2 = JSON.parse(localStorage.getItem('foundQMs_level2.2')) || {};
const gnomeWasClicked2 = localStorage.getItem('gnomeUnlocked_level2.2') === 'true';
  ...
  el.addEventListener('click', () => {
    if (!foundQMs2[obj.id]) {
      ...
      foundQMs2[obj.id] = true;
	  		    qmSound.currentTime = 0;
    qmSound.play();
      localStorage.setItem('foundQMs_level2.2', JSON.stringify(foundQMs2));
...
function showGnome2() {
  if (!document.getElementById('gnomeReveal2')) {
    ...
    gnomeImage2.src = 'backgrounds/level2/gnome2.png';
...
// clicking on gnome vvv
    ...
    gnomeImage2.addEventListener('click', () => {
      if (!localStorage.getItem('gnomeUnlocked_level2.2')) {
        localStorage.setItem('gnomeUnlocked_level2.2', 'true');

        const gnomes = JSON.parse(localStorage.getItem('gnomes')) || {};
        gnomes['level2.2'] = true;
        localStorage.setItem('gnomes', JSON.stringify(gnomes));
		    gnomeSound.currentTime = 0;
    gnomeSound.play();
 updateLevelIcons();
...
// clicking on gnome ^^^	
    ...
    if (localStorage.getItem('gnomeUnlocked_level2.2') === 'true') {
      gnomeImage2.classList.add('found');
	  updateLevelIcons(); // ✅ This triggers the icon update instantly
    }
